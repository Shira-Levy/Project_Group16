...
def manager_cancel_flight(flight_id):
    ...
    with db_curr() as cursor:
        ...
        if flight_dt < datetime.now() + timedelta(hours=72):
            flash("Error: Managers can cancel only up to 72 hours before the flight.", "error")
            return redirect(url_for("manager_flights"))
...
def manager_reports():
    ...
    f_year = request.args.get('filter_year')
    f_month = request.args.get('filter_month')
    ...
    if f_month:
        filter_conditions.append(f"MONTH(Date_of_flight) = {int(f_month)}")
    if f_aid:
        filter_conditions.append(f"A_ID = {int(f_aid)}")
